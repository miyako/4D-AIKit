...
An **instruct** model is trained, or fine-tuned, to follow instructions than simply engage in a casual conversaion. It uses its LLM training to determine what the users wants and responds accrodingly. It will only use tools when specifcally asked to. It is the right kind of AI to use as an interface for simple automation tasks. On the other hand, an instuct model may fail if the prompt is too abstract or lacks strategy.
